...
def PT(pnn, train_loader, valid_loader, args, msglogger, setup):
    ...
    if args.powerestimator == 'AL':
        pnn, best, lossfunction = al_train_pnn_progressive(
            pnn, train_loader, valid_loader, lossfunction, optimizer, args, msglogger, UUID=setup+'_PT')
    else:
...
def FT(train_loader, valid_loader, args, msglogger, setup):
    ...
    lossfunction.args.mu = initial_mu
...
for seed in range(10):
    ...
    setup = f"data_{datainfo['dataname']}_seed_{args.SEED}_Penalty_{
        args.powerestimator}_Factor_{int(args.powerbalance*1e2)}"
